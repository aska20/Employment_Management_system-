    ...
    const navigate = useNavigate();
     const handleChange = (e)=>{
        ...
        const handleSubmit = async (e)=>{
            e.preventDefault();
            try {
               const response = await axios.post(`${API_BASE}/api/department/add`,department,{
                headers:{
    ...
    <div className='max-w-3xl mx-auto mt-10 bg-white p-8 rounded-md shadow-md w-96'>
        <h2 className='text-2xl font-bold mb-6'>Add Department</h2>
            <form onSubmit={handleSubmit}>
                ...
                <button type="submit"
                className='w-full mt-6 bg-teal-600 hover:bg-teal-700 text-white font-bold py-3 px-4 rounded-xl transition'>Add Department</button>
            </form>
        </div>
